    ...
    <article className="group overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_80px_rgba(15,23,42,0.12)]">
      ...
      <div className="space-y-4 p-4 sm:space-y-5 sm:p-6">
        <div className="flex items-start justify-between gap-3 sm:gap-4">
          <div>
            <h3 className="text-base min-h-[60px] line-clamp-2 font-semibold text-slate-900 sm:text-xl">
              {card.title}
            </h3>
          ...
          <button className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-white transition hover:bg-slate-800 sm:h-12 sm:w-12">
            <FaArrowRight />
          </button>
        </div>
        ...
        <Link
          to={`/listing/${card.id}`}
          className="inline-flex w-full items-center justify-center rounded-2xl bg-slate-900 py-4 md:text-base font-semibold text-white transition hover:bg-slate-800"
        >
          {card.buttonText}
        </Link>
      </div>
