...
const ListingCard = ({ card }: ListingCardProps) => {
  return (
    ...
    <motion.article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm hover:shadow-lg">
      <div className="relative h-56 overflow-hidden">
        <img
          src={card.image}
          alt={card.title}
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="min-h-[60px] line-clamp-2 text-base font-bold text-[#021D49] sm:text-xl lg:text-2xl">
          {card.title}
        </h3>

        <p className="mt-2 min-h-[48px] line-clamp-2 text-sm text-gray-500 sm:text-base">
          {card.location}
        </p>

        <div
          className={`my-5 grid gap-4 text-sm sm:text-base ${
            card.secondaryValue ? "grid-cols-2" : "grid-cols-1"
          }`}
        >
          <div>
            ...
            <p className="line-clamp-2 font-bold text-sm text-[#021D49] sm:text-base">
              {card.priceValue}
            </p>
          </div>
          ...
          {card.secondaryValue && (
            <div>
              ...
              <p className="line-clamp-2 font-bold text-sm text-orange-500 sm:text-base">
                {card.secondaryValue}
              </p>
            </div>
          )}
        ...
        <Link
          to={`/listing/${card.id}`}
          className="mt-auto block w-full rounded-md bg-[#021D49] py-4 text-center text-sm font-semibold text-white transition duration-300 hover:bg-[#16356c] sm:text-base"
        >
          {card.buttonText || "View Details"}
        </Link>
      </div>
